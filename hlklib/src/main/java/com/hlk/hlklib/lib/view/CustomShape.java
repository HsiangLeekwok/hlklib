...
public class CustomShape extends View {
    ...
    private void initializeSetting(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.CustomShape, defStyleAttr, 0);
        ...
        if (_corners != _foreground_colors.length) {
            if (_corners < _foreground_colors.length) {
                int[] color = new int[_corners];
                System.arraycopy(_foreground_colors, 0, color, 0, _corners);
                _foreground_colors = new int[_corners];
                System.arraycopy(color, 0, _foreground_colors, 0, _corners);
            } else {
        ...
        if (_corners != _foreground_values.length) {
            if (_corners < _foreground_values.length) {
                int[] color = new int[_corners];
                System.arraycopy(_foreground_values, 0, color, 0, _corners);
                _foreground_values = new int[_corners];
                System.arraycopy(color, 0, _foreground_values, 0, _corners);
            } else {
    ...
    private void calculateValues() {
        ...
        int avg = total / _foreground_values.length;
        int half = max / 2;
        if (avg < half) avg = half;
        for (int i = 0, len = _foreground_values.length; i < len; i++) {
            int value = _foreground_values[i];
            _foreground_values[i] = value < avg ? avg : value;
        }
    }
